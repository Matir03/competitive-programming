...
//  freopen("C:\\Python27\\Lib\\site-packages\\visual\\examples\\out.txt","w",stdout);
	...
	for(i=1;i<n;++i)
	{
		...
		for(it = dist[p1].begin(); it != dist[p1].end(); it++)
			for(it1 = dist[p2].begin(); it1 != dist[p2].end(); it1++)
				dist[it->first][it1->first] = dist[it1->first][it->first] = it->second + it1->second + 1; 
					
		for(it = dist[p1].begin(); it != dist[p1].end(); it++)
			dist[it->first][p2] = dist[p2][it->first] = it->second + 1;
			
		for(it = dist[p2].begin(); it != dist[p2].end(); it++)
			if(dist[p1].count(it->first) == 0) dist[it->first][p1] = dist[p1][it->first] = it->second + 1;
